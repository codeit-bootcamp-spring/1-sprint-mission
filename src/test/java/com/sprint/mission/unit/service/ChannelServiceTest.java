...
public class ChannelServiceTest {
  ...
  void createPublicChannelSuccess() {
    ...
    when(channelRepository.save(any(Channel.class)))
        .thenAnswer(invocationOnMock -> reflectionFieldSetter.settingFieldValue(
  ...
  void findAllByUserIdSuccess() {
    ...
    when(channelRepository.findAllPrivateChannelByUserId(any(UUID.class))).thenReturn(
        privateChannelList);
    ...
    List<ChannelDto> privateChannels = allChannels.stream()
        .filter(channelDto -> channelDto.channelType().equals(PRIVATE)).toList();
    List<ChannelDto> publicChannels = allChannels.stream()
        .filter(channelDto -> channelDto.channelType().equals(PUBLIC)).toList();
