...
class BasicAuthLoginServiceTest {
    ...
    void login_Success() {
        ...
        UserDto userDto = new UserDto(user.getId(), user.getUsername(), user.getEmail(), user.getPhoneNumber(), user.getProfileId(), userStatus.isOnline(), user.getCreatedAt(), user.getUpdatedAt());
    ...
    void login_InvalidPassword() {
        ...
        // when
        when(authLoginValidator.findUserByUsername(request.username())).thenReturn(user);
        doThrow(InvalidResourceException.class).when(authLoginValidator).validatePassword(user, request.password());
