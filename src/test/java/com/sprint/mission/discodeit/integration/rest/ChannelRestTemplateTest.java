...
public class ChannelRestTemplateTest {
    ...
    void 사용자_채널_통합테스트() throws Exception {
        // 회원가입
        UserCreateRequestDto signupRequest = new UserCreateRequestDto("홍길동", "hong@test.com",
                "1234");
        ...
        HttpEntity<MultiValueMap<String, Object>> signupEntity = new HttpEntity<>(signupBody,
                signupHeaders);
        ResponseEntity<UserDto> createdUserResp = restTemplate.postForEntity(
        ...
        ChannelDto ch1 = restTemplate.postForEntity(
                ...
                new HttpEntity<>(new PublicChannelCreateRequestDto("channel-1", "desc-1"),
                        getJsonHeader()),
                ChannelDto.class).getBody();

        ChannelDto ch2 = restTemplate.postForEntity(
                getBaseUrl("/api/channels/public"),
                new HttpEntity<>(new PublicChannelCreateRequestDto("channel-2", "desc-2"),
                        getJsonHeader()),
                ChannelDto.class).getBody();
        ...
        ChannelDto[] remaining = restTemplate.getForEntity(
                getBaseUrl("/api/channels?userId=" + userId),
                ChannelDto[].class).getBody();
