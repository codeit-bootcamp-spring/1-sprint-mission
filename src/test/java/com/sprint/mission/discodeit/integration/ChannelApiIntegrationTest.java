...
class ChannelApiIntegrationTest {
  ...
  void createPublicChannel_Failure_InvalidRequest() throws Exception {
    // Given
    ChannelPublicRequest invalidRequest = new ChannelPublicRequest(
        "a", // 최소 길이 위반
        "테스트 채널 설명입니다."
    );
  ...
  void findAllChannelsByUserId_Success() throws Exception {
    ...
    // 공개 채널 생성
    ChannelPublicRequest publicChannelRequest = new ChannelPublicRequest(
        "공개 채널 1",
        "공개 채널 설명입니다."
    );
    ...
    ChannelPrivateRequest privateChannelRequest = new ChannelPrivateRequest(
