...
public class AWSS3Test {
  ...
  // 로거 직접 정의
  private static final Logger logger = LoggerFactory.getLogger(AWSS3Test.class);
  ...
  void downloadFromS3() {
    ...
    try {
      ...
      logger.info("다운로드된 파일 내용: {}", downloadedContent);
    } catch (S3Exception e) {
  ...
  void generatePresignedUrl() {
    ...
    try {
      ...
      logger.info("생성된 Presigned URL: {}", url);
    } catch (S3Exception e) {
      logger.error("Presigned URL 생성 실패: {}", e.getMessage());
      throw e;
    }
  }
  ...
  void cleanup() {
    try {
      ...
      logger.info("테스트 파일 정리 완료: {}", testKey);
    } catch (S3Exception e) {
      logger.error("테스트 파일 정리 실패: {}", e.getMessage());
    }
  }
}