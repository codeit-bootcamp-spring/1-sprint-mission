...
public class JavaApplication {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(basicUserService.readUser(user.getId()).getBinaryContent().getFilePath());
        basicUserService.updateUser(user.getId(),new UserUpdateDTO("홍감동", "1234", "dis@code.it", "filePath" ));

        System.out.println(basicUserService.readUser(user.getId()).getUserName());
        System.out.println(basicUserService.readUser(user.getId()).getBinaryContent().getFilePath());
