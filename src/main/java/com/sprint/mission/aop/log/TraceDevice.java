...
public class TraceDevice {
  ...
  public TraceStatus begin(ProceedingJoinPoint joinPoint) {
    ...
    log.info("[{}] {} {}(Method {}, Args {})", traceId.getId(),
        addSpace(START_PREFIX, traceId.getLevel()), simpleClassName, methodName,
        joinPoint.getArgs());
  ...
  private void complete(TraceStatus status, Throwable e, Object returnValue) {
    ...
    if (e == null) {
      log.info("[{}] {} {}(Method {}, Return {})", traceId.getId(),
          addSpace(COMPLET_PREFIX, traceId.getLevel()), simpleClassName, methodName, returnValue);
    } else {
      log.info("[{}] {} {}(Method {}) *예외 ={}", traceId.getId(),
          addSpace(EX_PREFIX, traceId.getLevel()), simpleClassName, methodName,
