...
public interface ChannelApiDocs {
  ...
  ResponseEntity<CustomApiResponse<ChannelResponse>> createPublicChannel(
  ...
  ResponseEntity<CustomApiResponse<ChannelResponse>> createPrivateChannel(
  ...
  ResponseEntity<CustomApiResponse<ChannelResponse>> updatePublicChannel(
      UUID channelId,
  ...
  ResponseEntity<CustomApiResponse<Void>> deleteChannel(UUID channelId);

  @Operation(summary = "유저의 채널 리스트", description = "유저가 접근할 수 있는 채널 리스트를 반환합니다.")
  @ApiResponses({
      @ApiResponse(responseCode = "200", description = "채널 조회 성공"),
  })
  ResponseEntity<CustomApiResponse<List<ChannelResponse>>> getChannelListByUser(UUID userId);
}
