...
public class CommonResponse<T> {
	...
	public static <T> CommonResponse<T> success(String message) {
		return new CommonResponse<>("200", message, null, HttpStatus.OK);
	}

	// 데이터와 함께 성공 응답 (200 OK)
	public static <T> CommonResponse<T> success(T data) {
		return new CommonResponse<>("200", "요청이 성공적으로 처리되었습니다.", data, HttpStatus.OK);
	}

	// 메시지와 데이터를 포함한 성공 응답 (200 OK)
	public static <T> CommonResponse<T> success(String message, T data) {
		return new CommonResponse<>("200", message, data, HttpStatus.OK);
	}

	// 생성 성공 응답 (201 CREATED)
	public static <T> CommonResponse<T> created(T data) {
		return new CommonResponse<>("201", "리소스가 성공적으로 생성되었습니다.", data, HttpStatus.CREATED);
	}
