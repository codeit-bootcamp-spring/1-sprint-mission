...
public enum ExceptionText {
    ...
    USER_NOT_FOUND("해당되는 사용자를 찾을 수 없습니다."),
    STATUS_NOT_FOUND("해당되는 Status를 찾을 수 없습니다."),
    STATUS_DUPLICATE("해당 사용자에 대한 UserStatus 가 이미 존재합니다."),
    INVALID_MESSAGE_CONTENT("메시지 내용은 null 또는 빈 값일 수 없습니다."),
    MESSAGE_CREATION_FAILED("메시지 생성 실패"),
    USER_CREATION_FAILED("유저 생성 실패"),
    USERSTATUS_CREATION_FAILED("UserStatus 생성 실패"),
    MESSAGE_NOT_FOUND("등록된 메시지가 없습니다."),
    NAME_DUPLICATE("이미 동일한 이름이 존재합니다. "),
    EMAIL_DUPLICATE("이미 동일한 이메일이 존재합니다."),
