...
public class SseService {
  ...
  public SseEmitter subscribe(UUID userId, String lastEventId) {
    ...
    emitter.onCompletion(() -> removeConnection(userId, wrapper));
    emitter.onTimeout(() -> removeConnection(userId, wrapper));
  ...
  public void sendToUser(UUID userId, String eventName, Object data) {
    ...
    for (SseEmitterWrapper wrapper : connectionsList) {
      try {
        ...
        log.error("SSE 이벤트 전송 실패: 사용자 ID = {}, 연결 ID = {}", userId, wrapper.getConnectionId(), e);
        removeConnection(userId, wrapper);
      }
    }
  }
  ...
  // 파일 업로드 상태 변경 이벤트 전송
  public void sendBinaryContentStatus(UUID userId, BinaryContentDto binaryContent) {
    sendToUser(userId, "binaryContents.status", binaryContent);
  }
  ...
  public void sendUserRefresh(UUID userId, UUID targetUserId) {
    ...
    sendToUser(userId, "users.refresh", data);
  }
