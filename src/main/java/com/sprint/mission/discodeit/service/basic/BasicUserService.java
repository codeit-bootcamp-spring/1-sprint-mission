...
public class BasicUserService implements UserService {
    ...
    public UserDto findUserById(UUID userId) {
        if (userId == null || userRepository.isUserExistByUUID(userId)==false) { System.out.println("유저 반환 실패. 입력값을 확인해주세요."); return null;}
    ...
    public UUID createUser(String userName, String email,String password, String profilePicturePath) {
        ...
        binaryContentRepository.addBinaryContent(BinaryContentType.Profile_Picture, null, binaryProfilePicture);
        //새로만든 유저객체에 프로필사진 등록
    ...
    public boolean deleteUser(UUID userId){
        ...
        if (binaryContentRepository.deleteBinaryContent(BinaryContentType.Profile_Picture, null, user.getProfilePictureId()) == false || userRepository.deleteUser(userId) == false || userStatusService.deleteUserStatus(userId) == false ){
            System.out.println("유저 삭제 과정 중 문제 발생"); return false; }
    ...
    public boolean changeProfilePicture(UUID userId, String profilePicturePath){
        ...
        //프로필사진 바이너리객체를 바이너리레포지토리에 저장.
        binaryContentRepository.addBinaryContent(BinaryContentType.Profile_Picture, null, uploadedProfilePicture);
        //유저객체에 프로필사진 등록
        User user = userRepository.getUserById(userId);
        if (user.getProfilePictureId() != null) { //기존 프로필사진은 DB에서 삭제
            binaryContentRepository.deleteBinaryContent(BinaryContentType.Profile_Picture, null, user.getProfilePictureId());
        }
