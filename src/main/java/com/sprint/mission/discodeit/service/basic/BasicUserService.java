...
public class BasicUserService implements UserService {
  ...
  public User createUser(CreateUserDto userDto) {
    ...
    validNickname(userDto.nickname());
    validPhone(userDto.phoneNumber());
  ...
  private void validNickname(String nickname) {
    ...
    List<User> users = userRepository.findAll();
    if (users.stream()
        .anyMatch(u -> u.getNickname().equals(nickname))) throw new UserValidationException(DUPLICATE_NICKNAME);
  }
  ...
  public void updateUser(String id, UserUpdateDto updatedUser, String originalPassword) {
    ...
    synchronized (originalUser) {
      if (updatedUser.nickname() != null) {
        validNickname(updatedUser.nickname());
        originalUser.setNickname(updatedUser.nickname());
      }
      if (updatedUser.email() != null) {
        validEmail(updatedUser.email());
        originalUser.setEmail(updatedUser.email());
      }
      if (updatedUser.phoneNumber() != null) {
        validPhone(updatedUser.phoneNumber());
