...
public class DiscodeitApplication {
    ...
    static Channel setupChannel(ChannelService channelService) {
        Channel channel = channelService.create(new CreateChannelDto("공지", ChannelType.PUBLIC, ChannelCategory.TEXT, "공지채널 입니다."));
        return channel;
    }
    ...
    public static void main(String[] args) {
        ...
        // 테스트
        messageCreateTest(messageService, channel, user.getId());
        ...
        System.out.println("\n\n--- Channel 등록---");
        Channel channel1 = channelService.create(new CreateChannelDto("일반1", ChannelType.PUBLIC, ChannelCategory.TEXT, "일반1 채널"));
        Channel channel2 = channelService.create(new CreateChannelDto("일반2", ChannelType.PUBLIC, ChannelCategory.TEXT, "일반2 채널"));
        Channel channel3 = channelService.create(new CreateChannelDto("공지사항", ChannelType.PUBLIC, ChannelCategory.TEXT, "공지사항 채널"));
        ...
        List<User> tempUsers = userService.findAll();
        ...
        User userByUUID = userService.findById(uuidStringForTest);
        UserResponseDto.from(userByUUID, userStatusService.findById(userByUUID.getUserStatusId()).isActive()).displayInfo();
        ...
        User userByEmail = userService.findByEmail("codeit@codeit.co.kr");
        UserResponseDto.from(userByEmail, userStatusService.findById(userByEmail.getUserStatusId()).isActive()).displayInfo();

        System.out.println("\n- 조회할 이메일을 가진 유저가 없는 경우");
        System.out.println("> no_one@test.com을 이메일로 가진 유저 조회 결과: ");
        try {

            User noUser = userService.findByEmail("no_one@test.com");
            UserResponseDto.from(noUser, userStatusService.findById(noUser.getUserStatusId()).isActive()).displayInfo();
        ...
        userService.findAllContainsNickname("박유진").forEach(u -> UserResponseDto.from(u, userStatusService.findById(u.getUserStatusId()).isActive()).displayInfo());
...
//        userService.getUserByUserStatus().forEach(User::displayInfo);
        ...
        channelService.findAllByChannelName("일반").forEach(Channel::displayFullInfo);


        System.out.println("\n4) 채널 type(종류)으로 조회(다건)");
        System.out.println("> Channel Type 이 PUBLIC 인 채널 조회 결과: ");
        channelService.findByChannelType(ChannelType.PUBLIC).forEach(Channel::displayFullInfo);

        System.out.println("\n5) 채널에 참여중인 유저 목록 조회");
        System.out.println("> channel1에 참여 중인 유저 목록 조회 결과:");
        channelService.findAllUserInChannel(channel1).forEach(u -> System.out.println(u.getId()));
        ...
        System.out.println("> 조회 결과:\n");
        messageService.findById(uuidStringForMessageTest).displayFullInfo();
        ...
        System.out.println("> 조회 결과:\n");
        try {
            messageService.findById(testUUID3.toString()).displayFullInfo();
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
        ...
        messageService.findAllContainsContent("반갑습니다").forEach(Message::displayFullInfo);


        System.out.println("\n4) 작성자로 조회(다건)");
        System.out.println("> user2(name=박유진)이 작성한 메세지 조회: ");
        messageService.findAllBySenderId(user2.getId()).forEach(Message::displayFullInfo);
        ...
        UserResponseDto.from(user1, userStatusService.findById(user1.getUserStatusId()).isActive()).displayInfo();
        //todo - 고민해볼것
        //UserResponseDto에 userStatus를 boolean으로 넣을 것인지, UserStatusId를 넣을 것인지.
        UpdateUserDto updateUserDto = new UpdateUserDto("codeit", user1.getEmail(), userStatusService.findById(user1.getUserStatusId()).isActive(), user1.getStatusMessage(), user1.getAccountStatus(), user1.getProfileImageId(), Instant.now());

        try {
            System.out.println("\n> 수정 결과: ");
            UserResponseDto.from(userService.updateUser(user1.getId(), updateUserDto), userStatusService.findById(user1.getUserStatusId()).isActive()).displayInfo();
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
        ...
        Message messageByUUID = messageService.findById(msg1.getId());
        ...
        User foundUserByEmail = userService.findByEmail("gildong@naver.com");
        System.out.println("\n-> User 삭제 결과: " + userService.deleteUser(foundUserByEmail.getId()));
